...
def generate_gaussian_noise(X_attack: np.ndarray,
    ...
    # Éviter std=0 pour les features constantes
...
def generate_camouflage(X_attack: np.ndarray,
    ...
    # Imiter les ports normaux (80, 443, 8080)
...
def generate_slow_and_low(n_samples: int = 100,
    ...
    CORRECTION : génère N_FEATURES colonnes (aligné avec FEATURE_NAMES)
    au lieu de 6 colonnes hardcodées.
    ...
    Retourne
    --------
    X_slow : np.ndarray de shape (n_samples, N_FEATURES)
    """
    np.random.seed(seed)

    # Initialiser toutes les features à zéro
    X_slow = np.zeros((n_samples, N_FEATURES))

    # Remplir les features clés avec des valeurs slow & low
    X_slow[:, IDX_DURATION]   = np.random.normal(90, 20, n_samples)   # Longue durée
    X_slow[:, IDX_BYTES_SENT] = np.random.normal(200, 50, n_samples)  # Très peu de bytes
    X_slow[:, IDX_BYTES_RECV] = np.random.normal(7_000, 2_000, n_samples)
    X_slow[:, IDX_PACKETS]    = np.abs(np.random.normal(5, 2, n_samples))  # Très peu de paquets
    X_slow[:, IDX_PORT]       = np.random.choice([80, 443], n_samples).astype(float)
    X_slow[:, IDX_PROTOCOL]   = 1.0  # TCP
...
def generate_protocol_flip(X_attack: np.ndarray,
    ...
    CORRECTION : seed utilisé pour la reproductibilité du copy.
